...
// The BTRCollectionViewLayout class is provided as an abstract class for subclassing to define custom collection layouts.
...
@property (nonatomic, getter=isHidden) BOOL hidden; // As an optimization, BTRCollectionView might not create a view for items whose hidden attribute is YES
...
@interface BTRCollectionViewLayoutAttributes(Private)
...
// To get the truth on the current state of the collection view, call methods on BTRCollectionView rather than these.
...
// BTRCollectionView calls these four methods to determine the layout information.
...
// The updateItems parameter is an array of BTRCollectionViewUpdateItem instances for each element that is moving to a new index path.

...
- (BTRControlState)state {
	...
	if (self.highlighted) state = state | BTRControlStateHighlighted;
	if (self.selected) state = state | BTRControlStateSelected;
	if (!self.enabled) state = state | BTRControlStateDisabled;
	return state;
}
